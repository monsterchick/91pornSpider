...
class PageHandler:
    ...
    def get_all_main_page_url(self):
        mainurl_list = self.mainurl_list
        ...
        while num_main_page < last_page:
            ...
            for index,div in enumerate(divs):
                ...
                for a_tag in a_tags:
                    ...
                    # print(f'page:{self.page} sublink: {self.sublink} {href}')
                    # print(f'page: {self.num_main_page} video: {index_of_video} subpage: {href}')
                    # self.sublink += 1
                    # get_m3u8(href)
                    # ts_list = self.get_m3u8(href)
                    # print(ts_list)
                ...
                for span_tag in span_tags:
                    ...
                    # print(f'page:{self.page} title:{title} url: {href}')#{sublink}
                    # global index_num_of_video
                    # index_num_of_video += 1
                    # self.download_videos(self.get_m3u8(href), title, self.video_id, self.page, self.index_num_of_video)
                    # print(title, href)
                    self.get_final_data(num_main_page, index_of_video, title, self.get_m3u8(href))

PageHandler().get_all_main_page_url()
